...
# Idé: Skapa .link-filer som hanteras som länkar. filnamnet visas, innehållet blir href. Förenklar flytt.
...
settings = {
	...
	'darkMode': 'half',
	'night':{'background-color': 'black',    'color': 'white','a-background-color': 'yellow','a-color': 'black',  'R':'red', 'G':'green', 'B':'yellow'},
	'half': {'background-color': 'darkgray', 'color': 'black','a-background-color': 'gray',  'a-color': 'yellow', 'R':'red', 'G':'green', 'B':'yellow'},
	'day':  {'background-color': 'white',    'color': 'black','a-background-color': 'yellow','a-color': 'black',  'R':'red', 'G':'green', 'B':'black'},

	'trans': {
		'<red>':   '<div style=color:red>',   '</red>': '</div>',
		'<green>': '<div style=color:green>', '</green>': '</div>',
		'<black>': '<div style=color:yellow>', '</black>': '</div>',
	},
	'showExt': False,
}

colors = settings[settings['darkMode']]
...
def writeHtmlFile(filename, title, content=""):
	...
	res.append("	<style> body { color:" + colors['color'] + "; background-color: " + colors['background-color'] + "; font-family:" + settings['fontFamily'] + "; font-size:" + settings['fontSize'] + "} </style>")
	res.append("	<style> a { padding-left:5px; padding-right:5px; text-decoration:none; color:" + colors['a-color'] + "; background-color:" + colors['a-background-color'] + " } </style>")
	res.append("	<style> div { margin-bottom:" + settings['margin'] + " } </style>")
	res.append("	<style> table, th, td { border: 1px solid " + colors['color'] + "; border-collapse: collapse; font-family:" + settings['fontFamily'] + "; font-size:" + settings['fontSize'] + "} </style>")
	res.append('</head>')
	res.append('<body>')
	res += [f"<h1>{title}</h1>"]
	res += [content]
...
def patch(s):
	# Reason: To have some whitespace between links (margin-bottom)
